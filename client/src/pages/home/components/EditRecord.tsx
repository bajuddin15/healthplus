...
const EditRecord: React.FC<IProps> = ({ patientRecord, fetchAllRecords }) => {
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      labName: patientRecord.labName || "",
      testType: patientRecord.testType || "",
      barcodeNumber: patientRecord.barcodeNumber || "",
      amountPaid: patientRecord.amountPaid || 0,
      amountDue: patientRecord.amountDue || 0,
      dateOfVisit: patientRecord?.dateOfVisit
        ? moment(patientRecord?.dateOfVisit).format("DD-MM-YYYY")
        : "",
      dateOfReport: patientRecord?.dateOfReport
        ? moment(patientRecord?.dateOfReport).format("DD-MM-YYYY")
        : "",
    }),
    [patientRecord]
  );
